...
rm(header, td_summary, ufo_extent, pts, esd_class)
```
...
```{r Classify median of plot values as Meeting of Failing, eval = F}
...
overall_stability <- left_join(dat2run, ss_median_all, by = c('ECO.SITE' = 'ESD')) %>% 
  mutate(STABILITY = if_else(is.na(STABILITY), 4, STABILITY), 
         Meeting = if_else(SiteStability >= STABILITY, T, F))

overall_stability <- cat_analysis(dframe = overall_stability, siteID = 'PLOTID', vars = 'Meeting',
             stratumID = 'ECO.SITE',  subpops = 'AoInference',  weight = 'WghtPerPlot',
             vartype = 'Local', conf = 80)
...
canopy_stability <- left_join(dat2run, ss_median_canopy, by = c('ECO.SITE' = 'ESD')) %>% 
  mutate(STABILITY = if_else(is.na(STABILITY), 4, STABILITY), 
         Meeting = if_else(SiteStability >= STABILITY, T, F))

canopy_stability <- cat_analysis(dframe = canopy_stability, siteID = 'PLOTID', vars = 'Meeting',
             stratumID = 'ECO.SITE',  subpops = 'AoInference',  weight = 'WghtPerPlot',
             vartype = 'Local', conf = 80)
...
interspace_stability_df <- left_join(dat2run, ss_median_interspace, by = c('ECO.SITE' = 'ESD')) %>% 
  mutate(STABILITY = if_else(is.na(STABILITY), 4, STABILITY), 
         Meeting = if_else(SiteStability >= STABILITY, T, F))

interspace_stability <- cat_analysis(dframe = interspace_stability_df, siteID = 'PLOTID', vars = 'Meeting',
             stratumID = 'ECO.SITE',  subpops = 'AoInference',  weight = 'WghtPerPlot',
             vartype = 'Local', conf = 80)
...
out <- WilsonCanHaveFun(data = interspace_stability, outcome = 'Meeting')
